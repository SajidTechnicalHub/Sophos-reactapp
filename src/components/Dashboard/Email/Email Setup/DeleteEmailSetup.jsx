...
const DeleteEmailSetup = (props) => {
    ...
    const DeleteEvent = (id) => {
        // Send id to EmalSetup.jsx handleDelete() so that data can be deleted by using current id
        props.DeleteEmailSetup(id)
        props.Cancel()
                ...
                <div className="delete-user-btn">
                    {
                        input.username === props.DeletedData.name ? <button className='delete' onClick={() => DeleteEvent(props.DeletedData.id)}>Delete</button> :
                            <button className='delete-disable' disabled >Delete </button>
...
export default DeleteEmailSetup